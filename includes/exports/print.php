<?php
	...
	class PDF extends FPDF{
		...
		function Header(){
			...
			if($this->PageNo()===1){
				...
				// Logo
				$this->Image('logo.png',10,10,0,10,'PNG');
				// Move to the right
				$this->Cell(10);
				// Title
				$this->Cell(14,5,$this->report_company,0,0,'L');
				// Move to the right
				$this->Cell(272);
				...
				// Globe Logo
				$this->Image('globe.png',295,8,0,12,'PNG');
				// Line break
				$this->Ln(6);
				$this->SetFont('Arial','',7);
				// Move to the right
				$this->Cell(10);
				// Address
				$this->Cell(14,0,$this->address,0,0,'L');			
				$this->Ln(3);
				$this->Cell(10);
				$this->Cell(14,0,$this->city,0,0,'L');			
			...
			if(!$this->is_last_page){
				...
				$header = array('Order No', 'Job Type','Service No', 'SubsName','Contact Number','Cabinet No.','Job Description','ApptDate','SlotFrom','SlotTo','Status','OK #','CBK','UY','SP');
				$border = array('LTB', 'LTB', 'LTB', 'LTB', 'LTB','LTB', 'LTB', 'LTB', 'LTB', 'LTB', 'LTB', 'LTB', 'LTB', 'LTB',  'LTBR');
				//Border
				//$w = array(15,10,16,35,25, 25, 40,15,9,9,7.2,50);
				$w = array(15,9.5,16,45,25, 25, 45,14.8,8.3,8.3,7.8,68,8,8,8);
				//Width		
				for($i=0;$i<count($header);$i++){
	...
	$widths = array(15,9.5,16,45,25, 25, 45,14.8,8.3,8.3,7.8,68,8,8,8,0);
	$pdf->SetWidths($widths);
	$pdf->SetFont('Arial','',7);
	$pdf->SetFillColor(243);
	$columns = array('ord_no', 'JobType','ServiceNo', 'FullName','ContactNo','CabinetNo','JobDescription','ApptDate','ApptSlotFrom','ApptSlotTo','HistoryStatus','OKNo','cbk','uy','sp');
