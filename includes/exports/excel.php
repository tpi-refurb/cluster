...
$alphabet = range('A', 'Z');
$cols = array('ord_no', 'JobType','ServiceNo', 'FullName','ContactNo','CabinetNo','JobDescription','ApptDate','ApptSlotFrom','ApptSlotTo','HistoryStatus','OKNo','cbk','uy','sp');
$summary_cols = array('ord_no', 'JobType','ServiceNo', 'FullName','ContactNo','CabinetNo','JobDescription','ApptDate','ApptSlotFrom','ApptSlotTo','status','OKNo');
$widths = array(8, 4,10, 20,12,14,20,9,5,5,5,30,5,5,5);
$summary_widths = array(8, 4,10, 20,12,14,20,9,5,5,20,30,5,5,5);
$colCount = count($cols);
...
if(count($rs)>0){
	...
	foreach($rs as $r){
		...
		$all_sheet->mergeCells('B'.$all_index.':K'.$all_index);
		...
		$all_sheet->mergeCells('B'.($all_index+1).':K'.($all_index+1));
		$all_sheet->setCellValueExplicit('L'.$all_index,'DATE',PHPExcel_Cell_DataType::TYPE_STRING);
		$all_sheet->setCellValueExplicit('L'.($all_index+1),$fdt,PHPExcel_Cell_DataType::TYPE_STRING);
		$all_sheet->getStyle('L'.($all_index+1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$all_sheet->mergeCells('L'.($all_index+1).':O'.($all_index+1));
		
		
		$all_sheet->getStyle('A'.$all_index.':A'.($all_index+1))->applyFromArray($team_outline);
		$all_sheet->getStyle('B'.$all_index.':K'.($all_index+1))->applyFromArray($team_outline);
		$all_sheet->getStyle('L'.$all_index.':O'.($all_index+1))->applyFromArray($team_outline);
		...
		if(count($rows)>0){
			...
			foreach($rows as $row){
				...
				foreach($cols as $col){
					...
					if($col==='OKNo'){
						$val = strtoupper($val);
						if(empty($val) or $val==='-' or $val==='N/A' or $val==='NA' or $val==='NONE'){
							$val = $row['status'];
						}else{
							$status = $row['status'];
							$val = $status.' '.$val;
			...
			$new_sheet_used_rows =$rowIndex;			
		}
		/* Set installer_1 name to sheet name */
		$sheet_title = $installername_1;
		$new_sheet->setTitle($sheet_title);
		
		/*Remove un-used rows in template */
		$rows_to_remove = 67-$new_sheet_used_rows;
		$new_sheet->removeRow($new_sheet_used_rows,$rows_to_remove+1);
	...
	$all_sheet->mergeCells('B'.$all_index.':D'.$all_index);
	...
	$all_sheet->mergeCells('B'.$all_index.':D'.$all_index);
	$all_sheet->mergeCells('G'.$all_index.':I'.$all_index);
	$all_sheet->getStyle('B'.$all_index.':D'.$all_index)->applyFromArray($border_bottom);
	$all_sheet->getStyle('G'.$all_index.':I'.$all_index)->applyFromArray($border_bottom);
	
	/* *****************************************************************
		START SUMMARY SHEETS 
	********************************************************************/
	$all_index = 5;
	/* create sheet for ALL */	
	$summary_sheet = $objPHPExcel->createSheet((count($rs)+1));	
	...
	$objDrawing->setHeight(52);
	$objDrawing->setCoordinates('A1');
	$objDrawing->setWorksheet($summary_sheet);
	...
	$is_printed_summaryheader = false;
	foreach($rs as $r){
		$assigned_to		= $r['installer_1'];
		$assigned_partner	= $r['installer_2'];
		$installername_1	= $r['Installer1'];
		$installername_2	= $r['Installer2'];
		$team_names = ($installername_1===$installername_2)? $installername_1: $installername_1.', '.$installername_2;
		
		/*Report Header */
		$summary_sheet->setCellValueExplicit('B1','Telcomtrix Phils., Inc.',PHPExcel_Cell_DataType::TYPE_STRING);
		$summary_sheet->mergeCells('B1:K1');
		$summary_sheet->getStyle('B1:K1')->applyFromArray($company_style);
		$summary_sheet->setCellValueExplicit('B2','241 Pasadena Dr., Santolan Rd.,',PHPExcel_Cell_DataType::TYPE_STRING);
		$summary_sheet->mergeCells('B2:K2');
		$summary_sheet->setCellValueExplicit('B3',' San Juan City, Manila, Philippines 1500',PHPExcel_Cell_DataType::TYPE_STRING);
		$summary_sheet->mergeCells('B3:K3');
		$summary_sheet->getStyle('B2:K3')->applyFromArray($address_style);
		
		$summary_sheet->setCellValueExplicit('L1','Accredited',PHPExcel_Cell_DataType::TYPE_STRING);
		$summary_sheet->setCellValueExplicit('L3','Contractor',PHPExcel_Cell_DataType::TYPE_STRING);
		$summary_sheet->getStyle('L1:L3')->applyFromArray($address_style);
		$summary_sheet->getStyle('L1:L3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
		
		$summary_sheet->setCellValueExplicit('A4','DAR ('.$fdt.')',PHPExcel_Cell_DataType::TYPE_STRING);
		$summary_sheet->mergeCells('A4:L4');		
		$summary_sheet->getStyle('A4:L4')->applyFromArray($company_style);
		$summary_sheet->getStyle('A4:L4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		
		/* Column Header */
		if($is_printed_summaryheader===false){
			$colIndx =0;
			$summary_sheet->setCellValueExplicit($alphabet[$colIndx].$all_index,'Team Name',PHPExcel_Cell_DataType::TYPE_STRING);
			$summary_sheet->getStyle($alphabet[$colIndx].$all_index)->applyFromArray($column_style);
			$colIndx =1;
			foreach($summary_cols as $col){
				$summary_sheet->setCellValueExplicit($alphabet[$colIndx].$all_index,$col,PHPExcel_Cell_DataType::TYPE_STRING);
				$summary_sheet->getStyle($alphabet[$colIndx].$all_index)->applyFromArray($column_style);
				$colIndx +=1;
			}
			$is_printed_summaryheader= true;
			$all_index +=1;
		}
		...
		if(count($rows)>0){
			...
			foreach($rows as $row){				
				$colIndx =0;				
				$summary_sheet->setCellValueExplicit($alphabet[$colIndx].$all_index,$team_names,PHPExcel_Cell_DataType::TYPE_STRING);
				$summary_sheet->getStyle($alphabet[$colIndx].$all_index)->getAlignment()->setWrapText(true);					
				$summary_sheet->getColumnDimension($alphabet[$colIndx])->setAutoSize(false);
				$summary_sheet->getColumnDimension($alphabet[$colIndx])->setWidth(18);
				$summary_sheet->getStyle($alphabet[$colIndx].$all_index)->applyFromArray($row_style);
				foreach($summary_cols as $col){					
					$val =$row[$col];					
					$summary_sheet->setCellValueExplicit($alphabet[$colIndx+1].$all_index,$val,PHPExcel_Cell_DataType::TYPE_STRING);
					$summary_sheet->getStyle($alphabet[$colIndx+1].$all_index)->getAlignment()->setWrapText(true);					
					$summary_sheet->getColumnDimension($alphabet[$colIndx+1])->setAutoSize(false);
					$summary_sheet->getColumnDimension($alphabet[$colIndx+1])->setWidth($summary_widths[$colIndx]);
					$summary_sheet->getStyle($alphabet[$colIndx+1].$all_index)->applyFromArray($row_style);
					$colIndx +=1;
				}
				$rowIndex +=1;
				$all_index +=1;				
	...
	$summary_sheet->setTitle("DAR-Summary");
	foreach($summary_sheet->getRowDimensions() as $rd) {
		$rd->setRowHeight(-1);
	}
	
	$all_index +=2; //Add row spacer every tech pair
	$summary_sheet->setCellValueExplicit('A'.$all_index,'Checked by:',PHPExcel_Cell_DataType::TYPE_STRING);
	$summary_sheet->setCellValueExplicit('F'.$all_index,'Approved by:',PHPExcel_Cell_DataType::TYPE_STRING);
	$summary_sheet->getStyle('A'.$all_index.':L'.$all_index)->applyFromArray($address_style);	
	$all_index +=2; //Add row spacer every tech pair
	
	$summary_sheet->setCellValueExplicit('B'.$all_index,$config->dispatch_checked_by,PHPExcel_Cell_DataType::TYPE_STRING);
	$summary_sheet->setCellValueExplicit('G'.$all_index,$config->dispatch_approved_by,PHPExcel_Cell_DataType::TYPE_STRING);
	$summary_sheet->mergeCells('B'.$all_index.':D'.$all_index);
	$summary_sheet->mergeCells('G'.$all_index.':I'.$all_index);
	$summary_sheet->getStyle('A'.$all_index.':L'.$all_index)->applyFromArray($team_style);
	$summary_sheet->getStyle('A'.$all_index.':L'.$all_index)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);	
	$all_index +=1; //Add row spacer every tech pair
	$summary_sheet->setCellValueExplicit('B'.$all_index,'Telcomtrix Representative',PHPExcel_Cell_DataType::TYPE_STRING);
	$summary_sheet->setCellValueExplicit('G'.$all_index,'Globe FO/  Representative',PHPExcel_Cell_DataType::TYPE_STRING);
	$summary_sheet->mergeCells('B'.$all_index.':D'.$all_index);
	$summary_sheet->mergeCells('G'.$all_index.':I'.$all_index);
	$summary_sheet->getStyle('B'.$all_index.':D'.$all_index)->applyFromArray($border_bottom);
	$summary_sheet->getStyle('G'.$all_index.':I'.$all_index)->applyFromArray($border_bottom);	
	/* *****************************************************************
		END SUMMARY SHEETS 
	********************************************************************/
