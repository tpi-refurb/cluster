<?php

	//header("Content-Type: text/json");
	
	require('../setup.php');
	
	
	$error	=array();	
	
	$cp	=(htmlspecialchars($_POST["ui_oldpassword"]));
	$uname = $auth->getUsername();
	$error = $auth->isAuthenticatedUser($uname,$cp);
	$userid = $auth->getUserid();
	$user = $auth->getUser($userid);
	$hashed_password =$user['password'];
	
	if (password_verify($cp, $hashed_password)) {
		$np	=(htmlspecialchars($_POST["ui_password"]));
		$rp	=(htmlspecialchars($_POST["ui_password_confirm"]));
		
		if(empty($np)){
			$error["error"]=true;
			$error['ui_password'] ='Enter new password';
		}
		if(empty($rp)){
			$error["error"]=true;
			$error['ui_password_confirm'] ='Re-type password';
		}else{
			if($rp !==$np){
				$error["error"]=true;
				$error['ui_password_confirm'] ='Password not match.';
			}
		}
		
		if($error["error"]===false){			
			$error = $auth->changePassword($userid, $cp, $np, $rp);
			if($error['error']===false){
				$error['message']='Password changed successfully.';
				$error["error"]=false;
			}else{
				$error["error"]=true;
				$error['message']='Changing password not success. Please try again.';
			}
		}else{
			$error["error"]=true;
			$error['message']='xxxxxxxxxxxxxxxxxxxxxxxxxxxx.';
		}
	}else{
		$error['message']='Please enter valid current password.';
	}
	echo json_encode($error);
?>